...
public class GelfMessageFactory {
    ...
    private static final String LOGGER_TYPE = "logType";
    ...
    public static GelfMessage makeMessage(Layout layout, LoggingEvent event, GelfMessageProvider provider) {
        ...
        //自定义的BUSINESS级别日志,加上额外字段logType:business
        if(Level.BUSINESS.equals(level)){
            gelfMessage.addField(LOGGER_TYPE,"business");
        }else if(Level.HEARTBEAT.equals(level)){
            gelfMessage.addField(LOGGER_TYPE,"heartbeat");
        }
